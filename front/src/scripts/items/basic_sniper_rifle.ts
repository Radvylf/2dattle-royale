...
export class BasicSniperRifle extends AbstractRifle {
    stats = {
        damage: 0,
        ...
        front_hand_dist: 0.04 + 1.375 / 2,
        proj_dist: 0.04 + 1.375 + 0.075 * Math.SQRT2
    };
    ...
    draw(display: Display, x: number, y: number, pointing_dir: number, shoot_anim_start: number): void {
        ...
        display.ctx.lineTo(...display.px(...display.shift_polar(...display.shift_polar(...display.shift_polar(x, y, pointing_dir, 0.04), pointing_dir + Math.PI / 2, 0.075), pointing_dir, 1.375)));
        display.ctx.lineTo(...display.px(...display.shift_polar(...display.shift_polar(x, y, pointing_dir, 0.04), pointing_dir, 1.375 + 0.075 * Math.SQRT2)));
        display.ctx.lineTo(...display.px(...display.shift_polar(...display.shift_polar(...display.shift_polar(x, y, pointing_dir, 0.04), pointing_dir - Math.PI / 2, 0.075), pointing_dir, 1.375)));
